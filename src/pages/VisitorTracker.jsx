...
const VisitorTracker = () => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, countryFilter, cityFilter, deviceFilter, dateRange]);
  ...
  const filteredVisitors = sortedVisitors.filter((v) => {
    ...
    if (dateRange.start) {
      matchDate =
        matchDate &&
        new Date(v.timestamp) >= new Date(dateRange.start + "T00:00:00");
    }
    if (dateRange.end) {
      matchDate =
        matchDate &&
        new Date(v.timestamp) <= new Date(dateRange.end + "T23:59:59");
    }

    return matchSearch && matchCountry && matchDevice && matchCity && matchDate;
  });
  ...
  const exportToCSV = () => {
    ...
    const rows = paginatedVisitors.map((v, idx) => [
